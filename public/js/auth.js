...
document.getElementById('a-changeemail').onclick = async () => {
  const r = await api('POST', '/api/auth/change-email', {
    newEmail: document.getElementById('a-newemail').value,
    currentPassword: document.getElementById('a-emailpass').value,
  });
  show('change email -> ' + r.status, r.data);
};
